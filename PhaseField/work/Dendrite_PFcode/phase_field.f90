...
subroutine cal_rkmat_pf_2d(K_R,phi,ad,uc,th,noise,ng_p,lo,hi,dx,thers,kk,Le,Mc00,&
  ...
  !A0 = 1.d0 - 3.d0 * anis
  !A0 = 1.d0 - 4.5d0 * anis!JC
  !A0 = 1.d0 - 49.5d0 * anis!JC
  A0 = 0.01
  !ebar = 4.d0 * anis / A0
  !ebar = 1.d0 * anis / A0!JC
  ebar = 0.02
  x_c = 1;y_c = 2;sd_id = 1;
  ...
  do j=lo(2),hi(2)
    do i=lo(1),hi(1)
      ...
      fA = 0.005d0**2 / 0.05d0 !JC da in comsol
      BT = 1.d0
      ...
      pfs = phi(i,j) * (1 - phi(i,j)) * (phi(i,j) - 0.5d0 + 12.5d0 * (uc(i,j) * exp(-38.6817d0 * 0.5d0 * th(i,j) + noise(i,j)) - &
            exp(38.6817 * 0.5d0 * th(i,j))) * phi(i,j) * (1.d0 - phi(i,j)))! JC source term ! add noise
      !pfs = phi(i,j) * (1 - phi(i,j)) * (phi(i,j) - 0.5d0 + 12.5d0 * (uc(i,j) * exp(-38.6817d0 * 0.5d0 * th(i,j))*noise(i,j) - &

      !K_R(i,j) = ( JR - JL + JT - JB ) / dx + &
       !        phi(i,j) * ( 1.d0-phi(i,j) * phi(i,j) )-( ( 1.d0 - phi(i,j) * phi(i,j) )**2 ) * pfs
      K_R(i,j) = ( JR - JL + JT - JB ) / dx + pfs!JC
      !K_R(i,j) = -1.d0 * ( JR - JL + JT - JB ) / dx + pfs!JC
      !K_R(i,j) = K_R(i,j)/(fA * BT)!JC
      K_R(i,j) = K_R(i,j)/fA
