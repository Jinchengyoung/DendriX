...
module potential_field_module
  ...
  public :: advance_potential_field

contains

subroutine advance_potential_field(mla,flw,phi_old,phi_new,dx,dt,the_bc_tower,pfpara)
    ...
    call advance_potential_field_den(mla,phi_old,phi_new,dx,dt,the_bc_tower,pfpara)
    

end subroutine advance_potential_field
...
subroutine advance_solute_field_den(mla,phi_old,phi_new,dx,dt,the_bc_tower,pfpara)
    ...
    do i=1,nfabs(phi_old(n))
       ...
       select case(dm)
       case (2)
          call cal_rkmat_uc_2d(pfn(:,:,1,3),pfo(:,:,1,1),pfn(:,:,1,1),pfo(:,:,1,3),ng_p, &
...
subroutine advance_potential_field_den(mla,phi_old,phi_new,dx,dt,the_bc_tower,pfpara)
    ...
    do i=1,nfabs(phi_old(n))
       ...
       select case(dm)
       case (2)
          call cal_rkmat_potential_2d(pfn(:,:,1,3),pfo(:,:,1,1),pfn(:,:,1,1),pfo(:,:,1,3),ng_p, &
                               lo,hi,dx(n),dt,pfpara%thers,pfpara%DM,pfpara%sk,&
                               -1.d0,0.d0)
    ...
    if (nlevs .eq. 1) then
       ...
       ! this includes periodic domain boundary ghost cells
       !call multifab_fill_boundary_c(phi_new(nlevs),2,1)
       call multifab_fill_boundary_c(phi_new(nlevs),3,1)!JC

       ! fill non-periodic domain boundary ghost cells
       !call multifab_physbc(phi_new(nlevs),2,1,1,the_bc_tower%bc_tower_array(nlevs))
       call multifab_physbc(phi_new(nlevs),3,1,1,the_bc_tower%bc_tower_array(nlevs))!JC
...
end subroutine advance_potential_field_den

subroutine cal_rkmat_potential_2d(K_R,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k,var_l,var_h)
  ...
  double precision  :: pt_old(lo(1)-ng_p:hi(1)+ng_p,lo(2)-ng_p:hi(2)+ng_p)
  ...
  do j=lo(2),hi(2)
    do i=lo(1),hi(1)
      ...
      call cal_sur_potential_flux_2d(x_c,JL,i,j,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
      call cal_sur_potential_flux_2d(x_c,JR,i+1,j,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
      call cal_sur_potential_flux_2d(y_c,JB,i,j,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
      call cal_sur_potential_flux_2d(y_c,JT,i,j+1,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
      ...
      pA = 4.5d0 * pf_old(i,j)**2 * (1 - pf_old(i,j))**2 * pft  ! JC source term in comsol
      pE = 1.d0 ! JC da in comsol

      K_R(i,j) = -1.d0 * ( JR - JL + JT - JB ) / dx + pA
      K_R(i,j) = K_R(i,j) / pE

      K_R(i,j) = pt_old(i,j) + dt * K_R(i,j)
...
end subroutine cal_rkmat_potential_2d
...
subroutine cal_rkmat_uc_2d(K_R,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k,var_l,var_h)
  ...
  do j=lo(2),hi(2)
    do i=lo(1),hi(1)
      ...
      call cal_sur_sol_flux_2d(x_c,JL,i,j,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
      call cal_sur_sol_flux_2d(x_c,JR,i+1,j,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
      call cal_sur_sol_flux_2d(y_c,JB,i,j,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
      call cal_sur_sol_flux_2d(y_c,JT,i,j+1,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
...
subroutine cal_sur_potential_flux_2d(s_case,flux,i,j,pf_old,pf_new,pt_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
  ...
  select case (s_case)
    case (1)
      ...
      !dfan_dx = ( pf_old(i,j) - pf_old(i-1,j) ) / dx
      !fan_p   = 0.5d0 * (pf_old(i,j) + pf_old(i-1,j))
      Js      = (-10.d0) * ((pt_old(i,j) - pt_old(i-1,j)) / dx) !JC FLUX
  ...
  case (2)
    ...
    Js      = (-10.d0) * ((pt_old(i,j) - pt_old(i,j-1)) / dx) !JC FLUX
...
end subroutine cal_sur_potential_flux_2d
...
subroutine cal_sur_sol_flux_2d(s_case,flux,i,j,pf_old,pf_new,uc_old,ng_p,lo,hi,dx,dt,thers,DM,p_k)
  ...
  select case (s_case)
    case (1)
      ...
      !dfan_dx = ( pf_old(i,j) - pf_old(i-1,j) ) / dx
      !fan_p   = 0.5d0 * (pf_old(i,j) + pf_old(i-1,j))
      Js      = (-2.25d0) * ((uc_old(i,j) - uc_old(i-1,j)) / dx) !JC FLUX
  ...
  case (2)
    ...
    !fan_p   = 0.5d0 * (pf_old(i,j) + pf_old(i,j-1))
    !Js      = DM * 0.5 * (1.d0 - fan_p) * (uc_old(i,j) - uc_old(i,j-1)) / dx
    Js      = (-2.25d0) * ((uc_old(i,j) - uc_old(i,j-1)) / dx) !JC FLUX
    !if (abs(dfan_dy) <= thers) then
    !    Ja = 0.d0
    !else
    !    dfan_dx = ( pf_old(i+1,j) + pf_old(i+1,j-1) - pf_old(i-1,j) - pf_old(i-1,j-1) ) / (4.d0 * dx)
...
end module potential_field_module
