  ...
  if(compute_mode .eq. 0) then
    ...
    if(pfpara%solve_flow_only /= 1) then
      ...
      do istep=1,nsteps_0
        ...
        if ( (mod(istep,status_int) .eq. 0) .and. parallel_IOProcessor()) then
            print*,'Dendrite Growth: Advancing time step',istep,'with dt=',dt
        end if
        ...
        ! advance potential field --JC
        call advance_potential_field(mla,flw,phi_old,phi_new,dx,dt,the_bc_tower,pfpara)
        ...
        if ( parallel_IOProcessor() .and. mod(istep,status_int) .eq. 0 ) then
            ! run time counting 
    ...
    call write_plotfile(mla,flw,ad_ori,phi_old,surf,nsteps_0,dx,time,prob_lo,prob_hi,pfpara)
